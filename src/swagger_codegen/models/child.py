...
class Child(Model):
    ...
    def validate(self):
        ...
        if self._grandchildren is not None and type(self._grandchildren) != list:
            errors.append({
                "code": "type_list",
                "field": "grandchildren",
                "message": "Invalid value for `grandchildren`, must not be type `list`"  # noqa: E501
            })
        if self._grandchildren is not None and type(self._grandchildren) == list:
            for i, attr_value in enumerate(self._grandchildren):  # pylint: disable=E1133
                attr_errors = attr_value.validate()
                for attr_error in attr_errors:
                    attr_error["field"] = "{parent_attr}[{index}]{child_attr}".format(
                        parent_attr="grandchildren", index=i, child_attr=attr_error["field"])
